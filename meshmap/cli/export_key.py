...
async def do_export_key(
    ...
    try:
        ...
        if result.type.value == 'private_key':
            ...
            click.echo(f"   Key length: {len(private_key_hex)} chars ({len(private_key_bytes)} bytes)")
            ...
            if len(private_key_bytes) == 64:
                ...
                if derived_pubkey_hex.lower() == self_pubkey.lower():
                    click.secho("✅ Key verification: Derived public key matches device!", fg='green')
                    click.echo(f"   Device public key: {self_pubkey[:16]}...{self_pubkey[-16:]}")
                else:
                    click.secho("⚠️  Warning: Derived public key doesn't match device public key", fg='yellow')
                    click.echo(f"   Derived: {derived_pubkey_hex[:16]}...{derived_pubkey_hex[-16:]}")
